    ...
    <main dir="rtl" style={pageStyle}>
      <section style={topBarStyle}>
        <div>
          ...
          <h1 style={pageTitleStyle}>إدارة الاشتراكات</h1>
          <p style={pageSubtitleStyle}>
            متابعة اشتراكات النشاطات، التمديد، التفعيل، الإيقاف، وتحديث بيانات الدفع.
          </p>
        </div>

        <span style={topBadgeStyle}>{subscriptions.length} اشتراك</span>
      </section>

      <section style={statsGridStyle}>
        <StatCard title="كل الاشتراكات" value={subscriptions.length} />
        <StatCard title="نشطة" value={activeCount} />
        <StatCard title="تجريبية" value={trialCount} />
        <StatCard title="غير نشطة" value={inactiveCount} />
        <StatCard title="إجمالي المبالغ" value={`${totalAmount.toFixed(2)} ريال`} />
      </section>
      ...
      <section style={createCardStyle}>
        <div>
          ...
          <p style={sectionSubtitleStyle}>
            تاريخ البداية يحفظ تلقائياً بتاريخ اليوم، وتاريخ الانتهاء يحسب حسب مدة الاشتراك المختارة.
          </p>
        </div>
      ...
      <section style={toolbarStyle}>
        <div>
          <h2 style={toolbarTitleStyle}>قائمة الاشتراكات</h2>
          <p style={toolbarTextStyle}>
            كل كرت يحتوي على بيانات الاشتراك، الدفع، التمديد، والتفعيل أو الإيقاف.
          </p>
        </div>
      </section>
      ...
      {subscriptions.length === 0 && !error ? (
        ...
        <section style={subscriptionsStackStyle}>
          {subscriptions.map((subscription) => (
            <article key={subscription.id} style={subscriptionCardStyle}>
              <div style={subscriptionMainStyle}>
                ...
                <aside style={sidePanelStyle}>
                  <div style={sideSummaryStyle}>
                    ...
                    <strong>{statusLabel[subscription.status] || subscription.status}</strong>
                  </div>
...
const subscriptionsStackStyle: React.CSSProperties = {
  display: "grid",
  gap: "16px",
};
...
const subscriptionMainStyle: React.CSSProperties = {
  display: "grid",
  gridTemplateColumns: "1fr 240px",
  gap: "18px",
};

const subscriptionInfoStyle: React.CSSProperties = {
  minWidth: 0,
};
